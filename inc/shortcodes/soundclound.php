...
add_shortcode("soundcloud", "luxicar_toolkit_soundcloud_shortcode");
 ...
 */
function luxicar_toolkit_soundcloud_shortcode($atts, $content = null) {
  ...
  $player_type = luxicar_toolkit_soundcloud_booleanize('player_type', 'visual');
  ...
  $plugin_options = array_filter(array(
    'iframe' => $isIframe,
    'width'  => luxicar_toolkit_soundcloud_booleanize('player_width'),
    'height' => luxicar_toolkit_soundcloud_url_has_tracklist($shortcode_options['url']) ? luxicar_toolkit_soundcloud_booleanize('player_height_multi') : luxicar_toolkit_soundcloud_booleanize('player_height'),
    'params' => array_filter(array(
      'auto_play'     => luxicar_toolkit_soundcloud_booleanize('auto_play'),
      'show_comments' => luxicar_toolkit_soundcloud_booleanize('show_comments'),
      'color'         => luxicar_toolkit_soundcloud_booleanize('color'),
  ...
  $iframe = luxicar_toolkit_get_soundcloud_booleanize($options['iframe']);

  // Remove visual parameter from Flash widget or when it's false because that's the default
  if ($options['params']['visual'] && (!$iframe || !luxicar_toolkit_get_soundcloud_booleanize($options['params']['visual']))) {
  ...
  if ($iframe) {
    return luxicar_toolkit_soundcloud_iframe_flash_widget($options);
  } else {
    return luxicar_toolkit_soundcloud_flash_widget($options);
  }
 ...
 */
function luxicar_toolkit_soundcloud_booleanize($option, $default = false) {
 ...
 * @return {boolean}
 */
function luxicar_toolkit_get_soundcloud_booleanize($value) {
 ...
 * @return {boolean}
 */
function luxicar_toolkit_soundcloud_url_has_tracklist($url) {
 ...
 */
function luxicar_toolkit_soundcloud_oembed_params_callback($match) {
 ...
 */
function luxicar_toolkit_soundcloud_iframe_flash_widget($options) {
  ...
  $height = isset($options['height']) && $options['height'] !== 0
              ? $options['height']
              : (luxicar_toolkit_soundcloud_url_has_tracklist($options['url']) || (isset($options['params']['visual']) && luxicar_toolkit_get_soundcloud_booleanize($options['params']['visual'])) ? '450' : '166');
 ...
 */
function luxicar_toolkit_soundcloud_flash_widget($options) {
  ...
  $width = isset($options['width']) && $options['width'] !== 0 ? $options['width'] : '100%';
  // Set default height if not defined
  $height = isset($options['height']) && $options['height'] !== 0 ? $options['height'] : (luxicar_toolkit_soundcloud_url_has_tracklist($options['url']) ? '255' : '81');
